...
def ce_jobrun(CE_DATA, logdna_ingestion_key, logdna_region, apikey, cos_crn, cos_bucket, cos_endpoint,
              key_first_logged):
    ...
    if CE_DATA:
        ...
        if cos_bucket != ce_data["bucket"]:
            log.error(
                f"misconfigured bucket in config map in the environment does not match the bucket supplied by the COS subscription, COS_BUCKET:{cos_bucket},  CE_DATA[bucket]:{ce_data['bucket']}")
            cos_bucket = ce_data["bucket"]
        key = ce_data["key"]
        log.info(f"COS bucket:{cos_bucket}, key:{key}")
        lib.log_cos_object_simple(logdna_endpoint, logdna_ingestion_key, apikey, cos_crn, cos_endpoint, cos_bucket, key,
                                  key_first_logged)
    else:
        lib.log_all_cos_objects_simple(logdna_endpoint, logdna_ingestion_key, apikey, cos_crn, cos_endpoint, cos_bucket,
                                       key_first_logged)
...
def ce_job():
    ...
    log.info(
        f"COS_CRN:{ce_environ.COS_CRN} COS_BUCKET:{ce_environ.COS_BUCKET} COS_ENDPOINT:{ce_environ.COS_ENDPOINT} LOGDNA_REGION:{ce_environ.LOGDNA_REGION} KEY_FIRST_LOGGED:{ce_environ.KEY_FIRST_LOGGED}")
    CE_DATA = os.getenv("CE_DATA", None)
    log.info(f"CE_DATA:{CE_DATA}")
    ce_jobrun(CE_DATA, ce_environ.LOGDNA_INGESTION_KEY, ce_environ.LOGDNA_REGION, ce_environ.APIKEY, ce_environ.COS_CRN,
              ce_environ.COS_BUCKET, ce_environ.COS_ENDPOINT, ce_environ.KEY_FIRST_LOGGED)
