...
import json_log_converter


log = logging.getLogger("flowlog")
LOG_FORMAT_JSON = os.getenv("LOG_FORMAT_JSON", False)
...
class CosObject:
    ...
    def non_fatal(self) -> bool:
        return (not self.good()) and isinstance(self.exception, ibm_botocore.exceptions.ClientError) and \
            self.exception.response['Error']['Code'] == "NoSuchKey"
        ...
        if cos_obj.good():
            ...
            for row_json in rows_from_gz_flowlog_stream(obj["Body"], key):
                if LOG_FORMAT_JSON:
                    ...
                    out_str = json_log_converter.convert_log_plain(row_json)
    ...
    def older(self, key: str):
      ...
      return head and consider it older"""
        ...
        if self.head_first_logged_object == None:
            ...
            return KeyHeadOlder(key, head,
                                head['Metadata']['capture_start_time'] < self.head_first_logged_object['Metadata'][
                                    'capture_start_time'])
...
def log_cos_object_and_remember(logdna_endpoint, logdna_ingestion_key, apikey, cos_crn, cos_endpoint, bucket, key,
                                key_first_logged):
...
def log_all_cos_objects_simple(logdna_endpoint, logdna_ingestion_key, apikey, cos_crn, cos_endpoint, bucket,
                               key_first_logged):
...
def log_cos_object_simple(logdna_endpoint, logdna_ingestion_key, apikey, cos_crn, cos_endpoint, bucket, key,
                          key_first_logged):
    log_cos_object(logdna_endpoint, logdna_ingestion_key, apikey, cos_crn, cos_endpoint, bucket, key)
